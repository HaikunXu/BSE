#' xxx
#' 
#' \code{create.strat.flg.f} yyy
#' 
#' @export

create.strat.flg.f = function(lat.5deg,lon.5deg,is.lwrght,month,setype,vessel.class) {
  # from input vectors values, assigns each record to catch estimation stratum 
  #   catch estimation strata are defined by: set type-vessel size class(1-5;6) x area x month 
  #
  # function does not stratify by year
  #
  # is.lwrght is a T/F value that indicates whether the input 5 deg lat and lon are lower right corner (T) or square center (F)
  #
  # NOTE: spatial part stratum definitions are based on 5 degree square center 
  #
  #
  print("*** Please edit first to make sure stratum definitions are correct ***")
  #
  # number of records in input vectors
  nrecs<-length(lat.5deg)
  #
  # initialize and fill gear vector
  gear<-rep(NA,nrecs)
  gear[setype==1 & vessel.class<=5]<-4
  gear[setype==1 & vessel.class==6]<-7
  gear[setype==4 & vessel.class<=5]<-3
  gear[setype==4 & vessel.class==6]<-6
  gear[setype==5 & vessel.class<=5]<-2
  gear[setype==5 & vessel.class==6]<-5
  #
  # initialze and fill area vector
  # area<-rep(NA,nrecs)
  #
  # if lat.5deg and lon.5deg are lower right corner, convert these to 5 degree square center
  if(is.lwrght){
    lat.5deg<-lat.5deg+2.5
    lon.5deg<-lon.5deg-2.5
  }
  #
  ######
  #
  # SKJ Mark assessment for SAC 2022
  #
  # Catch areas for SKJ in DEL
  # print("Using catch stratification: SKJ Mark assessment 2022 for DEL")
  # area<-rep(1,nrecs)
  # area[lat.5deg<=(-2.5)]<-2
  #
  # Catch areas for SKJ in UNA
  # print("Using catch stratification: SKJ Mark assessment 2022 for UNA")
  # area<-rep(1,nrecs)
  # area[lon.5deg>(-122.5) & lon.5deg<=(-87.5) & lat.5deg<=(17.5)]<-2
  # area[lon.5deg>(-122.5) & lat.5deg>(17.5)]<-3
  # area[lon.5deg>(-87.5)]<-4
  #
  # Catch areas for SKJ in OBJ
  print("Using catch stratification: SKJ Mark assessment 2022 for OBJ")
  area<-rep(1,nrecs)
  area[lon.5deg>(-122.5) & lat.5deg<=(-12.5)]<-2
  area[lon.5deg>(-122.5) & lon.5deg<=(-102.5) & lat.5deg>(-12.5)]<-3
  area[lon.5deg>(-102.5) & lat.5deg>(-12.5)]<-4
  #
  ##
  # Fishery area-gears for SKJ in DEL (UNA and OBJ are junk)
  # print("Using fishery stratification: SKJ Mark assessment 2022 for DEL")
  # fishery.areagear<-rep(NA,nrecs)
  # fishery.areagear[(gear==2 | gear==5) & area==1]<-"FO.A1"
  # fishery.areagear[(gear==2 | gear==5) & area>1]<-"FO.A2"
  # #
  # fishery.areagear[(gear==3 | gear==6) & area==1]<-"UN.A1"
  # fishery.areagear[(gear==3 | gear==6) & area>1]<-"UN.A2"
  # #
  # fishery.areagear[(gear==4 | gear==7) & area==1]<-"DP.A1"
  # fishery.areagear[(gear==4 | gear==7) & area==2]<-"DP.A2"
  #
  # Fishery area-gears for SKJ in UNA (DEL and OBJ are junk)
  # print("Using fishery stratification: SKJ Mark assessment 2022 for UNA")
  # fishery.areagear<-rep(NA,nrecs)
  # fishery.areagear[(gear==2 | gear==5) & area==1]<-"FO.A1"
  # fishery.areagear[(gear==2 | gear==5) & area>1]<-"FO.A2"
  #
  # fishery.areagear[(gear==3 | gear==6) & area==1]<-"UN.A1"
  # fishery.areagear[(gear==3 | gear==6) & area==2]<-"UN.A2"
  # fishery.areagear[(gear==3 | gear==6) & area==3]<-"UN.A3"
  # fishery.areagear[(gear==3 | gear==6) & area==4]<-"UN.A4"
  #
  # fishery.areagear[(gear==4 | gear==7) & area==1]<-"DP.A1"
  # fishery.areagear[(gear==4 | gear==7) & area>1]<-"DP.A2"
  #
  # Fishery area-gears for SKJ in OBJ (DEL and UNA are junk)
  print("Using fishery stratification: SKJ Mark assessment 2022 for OBJ")
  fishery.areagear<-rep(NA,nrecs)
  fishery.areagear[(gear==2 | gear==5) & area==1]<-"FO.A1"
  fishery.areagear[(gear==2 | gear==5) & area==2]<-"FO.A2"
  fishery.areagear[(gear==2 | gear==5) & area==3]<-"FO.A3"
  fishery.areagear[(gear==2 | gear==5) & area==4]<-"FO.A4"

  fishery.areagear[(gear==3 | gear==6) & area==1]<-"UN.A1"
  fishery.areagear[(gear==3 | gear==6) & area>1]<-"UN.A2"

  fishery.areagear[(gear==4 | gear==7) & area==1]<-"DP.A1"
  fishery.areagear[(gear==4 | gear==7) & area>1]<-"DP.A2"
  #
  ######
  # return stratum id data frame
  stratum.id<-data.frame(area,month,gear,fishery.areagear)
  stratum.id$fishery.areagear<-as.character(stratum.id$fishery.areagear)
  
  # plot area code to double-check
  plot(x=lon.5deg[stratum.id$area==1],y=lat.5deg[stratum.id$area==1],pch="1",
       xlim=c(min(lon.5deg),max(lon.5deg)),ylim=c(min(lat.5deg),max(lat.5deg)),
       xlab="Lon",ylab="Lat")
  if(length(unique(stratum.id$area))>1) {
    for (a in 2:length(unique(stratum.id$area))) {
      points(x=lon.5deg[stratum.id$area==a],y=lat.5deg[stratum.id$area==a],pch=toString(a))
    }
  }
  
  return(stratum.id)
}