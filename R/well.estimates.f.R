#' xxx
#' 
#' \code{well.estimates.f} yyy
#' 
#' @export

well.estimates.f = function(lfgrpd.master,lfmm.frm) {
  # this function computes within-well estimates (see Tomlinson document in SAR4 2004): W_j; What_ij; Nhat_j; Nhat_ij; Nhat_ijk
  # 
  # lfgrpd.master is output from read.lengthfrq.f; lfmm.frm is output from read.lfmmdata.f
  #
  # species look-up table
  spp.lookup<-matrix(c(106,110,111,1,2,3),ncol=2,nrow=3,byrow=F)
  #
  # subset grouped data and create grouped well sample id for samples with bet and/or yft and/or skj
  lfgrpd.frm<-lfgrpd.master[lfgrpd.master$species==106 | lfgrpd.master$species==110 | lfgrpd.master$species==111,]
  # sample ids are trip number:well sample number
  sampid.grpd<-paste(lfgrpd.frm$tripno,lfgrpd.frm$wellsampno,sep=":")
  #
  ...
  #
  # create well sample id for lf mm data
  ...
  Nhat.j<-rep(0,nsamps)
  lf.well.frm<-data.frame(matrix(NA,nrow=nsamps,ncol=12))
  names(lf.well.frm)<-c("tripno","wellsampno","vesno","flag","year","month","class","setype","lat.5deg","lon.5deg","wellmt.3spp","sorted")
  #
  #
  ...
  for(i in 1:nsamps){
    ...
    tmpdat<-lfgrpd.frm[sampid.grpd==sampid.grpd.unq[i],]
    #
    # store ancillary information
    lf.well.frm$tripno[i]<-tmpdat$tripno[1]
    lf.well.frm$wellsampno[i]<-tmpdat$wellsampno[1]
    lf.well.frm$vesno[i]<-tmpdat$vesno[1]
    lf.well.frm$flag[i]<-tmpdat$flag[1]
    lf.well.frm$year[i]<-tmpdat$year.firstset[1]
    lf.well.frm$month[i]<-floor(tmpdat$moda.firstset[1]/100)
    lf.well.frm$class[i]<-tmpdat$class[1]
    lf.well.frm$setype[i]<-tmpdat$setype[1]
    lf.well.frm$lat.5deg[i]<-tmpdat$lat.5deg[1]
    lf.well.frm$lon.5deg[i]<-tmpdat$lon.5deg[1]
      ...
      if(sortid==2){
        ...
        for(k in 1:nspp){
          ...
          if(!is.na(tmpdat$sample.size[k]) & tmpdat$sample.size[k]>0){
            sppkg[spp.lookup[spp.lookup[,1]==tmpdat$species[k],2]]<-sum(lfmm.frm$weight.kg[sampid.lfmm==sampid.grpd.unq[i] & lfmm.frm$spp==tmpdat$species[k]])
            #
            tmpcnts<-table(lfmm.frm$length.1cmbin[sampid.lfmm==sampid.grpd.unq[i] & lfmm.frm$spp==tmpdat$species[k]])
      ...
      if(sortid==1){
        ...
        for(mm in 1:nspp){
          ...
          for(kj in 1:nsplits.spp){
            ...
            #   note: assume that pcnt.totalcatch gives percent of total well catch that was in split and
            #   that split is for a single species.
            wellwt.split<-tmpdat.spp$totalmt.catch[kj]*(tmpdat.spp$pcnt.totalcatch[kj]/100)
            sampwt.split<-sum(lfmm.frm$weight.kg[lfmm.frm$tripno==tmpdat.spp$tripno[1] & lfmm.frm$wellsampno==tmpdat.spp$wellsampno[1] & lfmm.frm$sampno==tmpdat.spp$sampno[kj] & lfmm.frm$spp==tmpdat.spp$species[1]])
            nmsrd.split<-length(lfmm.frm$length.1cmbin[lfmm.frm$tripno==tmpdat.spp$tripno[1] & lfmm.frm$wellsampno==tmpdat.spp$wellsampno[1] & lfmm.frm$sampno==tmpdat.spp$sampno[kj] & lfmm.frm$spp==tmpdat.spp$species[1]])
            ...
            tmpcnts<-table(lfmm.frm$length.1cmbin[lfmm.frm$tripno==tmpdat.spp$tripno[1] & lfmm.frm$wellsampno==tmpdat.spp$wellsampno[1] & lfmm.frm$sampno==tmpdat.spp$sampno[kj] & lfmm.frm$spp==tmpdat.spp$species[1]])
