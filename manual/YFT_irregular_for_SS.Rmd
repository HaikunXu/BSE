...
```{r plot comp data - 2}
names(YFT_PS_Comp)[5:205] <- 1:201
YFT_PS_Comp_plot <- YFT_PS_Comp %>%
  gather(5:205,key="Length",value=comp) %>%
  mutate(Length = as.numeric(Length))

ggplot(data=YFT_PS_Comp_plot %>% filter(Type == "DEL")) +
  geom_tile(aes(x=Year,y=Length,fill=comp)) +
    scale_fill_gradient(
    low = "white",
    high = "red",
    na.value = "grey",
  ...
  coord_cartesian(xlim = c(80,120), ylim = c(50,150)) +
