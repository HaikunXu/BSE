...
save(list=objects(pat="fishery.estimates"),file=paste0(save_dir,"BET_",PS,"_2000-2021.new.RData"))
...
# save results
write.csv(BET.OBJ.Catch.20002021,file=paste0(save_dir,"BET.OBJ.Catch.20002021.new.csv"),row.names = FALSE)
write.csv(BET.OBJ.Comp.20002021,file=paste0(save_dir,"BET.OBJ.Comp.20002021.new.csv"),row.names = FALSE)
...
BET.OBJ.Catch.20002021.new <- read.csv(paste0(save_dir,"BET.OBJ.Catch.20002021.new.csv"))
...
BET.OBJ.Catch.20002021.new <- BET.OBJ.Catch.20002021.new %>% mutate(Source="Haikun.new")

BET_OBJ_Catch <- rbind(BET_OBJ_Catch.SAC12,BET.OBJ.Catch.20002021,BET.OBJ.Catch.20002021.new) %>% 
  mutate(Year=year+quarter/4-1/8) %>%
  filter(year>1999,year<2020) %>%
  mutate(EPO=A1+A2+A3+A4+A5) %>%
  gather(c(3:7,10),key="Area",value="Catch") %>%
  group_by(year,Area,Source) %>%
  mutate(Catch_annual=sum(Catch))

ggplot(data=BET_OBJ_Catch) +
  geom_line(aes(x=year,y=Catch_annual,color=Source)) +
  facet_wrap(~Area,nrow=5,scales="free_y") +
  theme_bw()
