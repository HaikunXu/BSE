...
area.substitution.mat <- area.substitution.mat.YFT.DEL.2019ExtRev # for OBJ
...
YFT.OBJ.Catch.20002021<-compile.catch.output.f(yr.start,yr.end,PS=PS,Species=Species,c("A1","A2","A3","A4","A5"))
...
YFT_OBJ_Catch.SAC12 <- read.csv("D:/OneDrive - IATTC/IATTC/2022/BSE stuff from Cleridy/spp comp programs_from 2000/current_estimates/YFT_FLOATING-OBJECT CATCH_FORMATTED_1975-2019_for SAC 2020.csv")
YFT.OBJ.Catch.20002021 <- read.csv(paste0(save_dir,"YFT.OBJ.Catch.20002021.csv"))

YFT_OBJ_Catch.SAC12 <- YFT_OBJ_Catch.SAC12[,2:8] %>% mutate(Source="Cleridy")
YFT.OBJ.Catch.20002021 <- YFT.OBJ.Catch.20002021 %>% mutate(Source="Haikun")

YFT_OBJ_Catch <- rbind(YFT_OBJ_Catch.SAC12,YFT.OBJ.Catch.20002021) %>% 
...
ggsave(file=paste0(save_dir,"YFT.OBJ.Catch.png"),h=10,w=10)
